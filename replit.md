...
- **Stock Availability:** The products API enriches each product with `available_stock` count (unsold, enabled stock items). VPS products return `null` (always available). If stock query fails, returns `null` (unknown) to avoid false out-of-stock. All dates use Asia/Jakarta (WIB) timezone.
