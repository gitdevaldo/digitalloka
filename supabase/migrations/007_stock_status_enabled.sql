...
ALTER TABLE product_stock_items
  ...
  CHECK (status IN ('unsold', 'sold', 'disabled', 'enabled'));

UPDATE product_stock_items
  SET status = 'enabled'
  WHERE status = 'unsold' AND is_unlimited = true;
...
BEGIN
  ...
  IF p_stock_item_id IS NOT NULL THEN
    ...
    WHERE id = p_stock_item_id
      ...
      AND (status IN ('unsold', 'enabled') OR (status = 'sold' AND is_unlimited = TRUE))
    FOR UPDATE;
  ELSE
    SELECT * INTO v_stock
    FROM product_stock_items
