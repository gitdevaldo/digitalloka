...
export default function EditVpsStockPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      setRegion((meta.region as string) || '');
      setOs((meta.os as string) || '');
  ...
  useEffect(() => {
    if (!provider || !providerLoaded) return;
  ...
  async function handleSave() {
    ...
    try {
      ...
      const updatedMeta = {
        ...
        os: os || undefined,
        region_name: regions.find(r => r.slug === region)?.name || (existingMeta.region_name as string) || region || undefined,
        os_name: images.find(i => i.slug === os)?.name || (existingMeta.os_name as string) || os || undefined,
        ...
        <div style={{ display: 'grid', gap: 20, alignContent: 'start' }}>
          ...
          <Panel title="Preview">
            <div style={{ background: 'var(--muted)', borderRadius: 'var(--r-sm)', padding: 16 }}>
              ...
              {(selectedRegionName || selectedOsName || (region && !selectedRegionName) || (os && !selectedOsName)) && (
                <div className="flex gap-1.5 flex-wrap mb-3">
                  {(selectedRegionName || region) && (
                    <span className="inline-flex bg-accent/10 text-accent rounded text-[0.68rem] font-bold px-2 py-0.5 border border-accent/30">{selectedRegionName || region}</span>
                  )}
                  {(selectedOsName || os) && (
                    <span className="inline-flex bg-quaternary/10 text-foreground rounded text-[0.68rem] font-bold px-2 py-0.5 border border-quaternary/30">{selectedOsName || os}</span>
                  )}
                </div>
              )}
